...
const screenForNames = (arr) => {
  // Solution code here...
  let regex = /^(?:Mr\.|Mrs\.|Ms\.|Dr\.) [A-Za-z]+(?: [A-Za-z]+)*$/gm;
  return arr.filter(v => regex.test(v));
};
...
const toTitleCase = (arr) => {
  // Solution code here...
  return arr.map(v => v.charAt(0).toUpperCase() + v.slice(1));
};
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  return arr.filter(v => parseInt(arr[0].mass) < parseInt(v.mass)).map(v => v.name ).join(' - ');
};
...
const sortBy = (property, arr) => {
  // Solution code here...
  return arr.sort((a, b) => a[property] - b[property] || a[property].localeCompare(b[property]));
};
...
const detectTicTacToeWin = (board) => {
  // Solution code here...
  // I had to heavily use chatGPT for this one and still couldn't figure it out by myself and still sorta don't understand it but I got it.
  const checkWinCondition = (cells) => {
    const firstCell = cells[0];
    if (firstCell !== '') {
      return cells.every((cell) => cell === firstCell);
    }
    return false;
  };
  ...
  if (checkWinCondition(diagonal1) || checkWinCondition(diagonal2)) {
    return true;
  }
