...
const count = (target, input) => {
  // Solution code here...
  return input.reduce((acc, v) => acc + v.filter(num => num === target).length, 0);
};
...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce((acc, v) => acc + v.reduce((acc2, e) => acc2 + e, 0), 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(a => {
    return a.filter(b => typeof (b) === 'number' && b % 5 === 0)
      .map(c => Math.pow(2, c));
  });
};
