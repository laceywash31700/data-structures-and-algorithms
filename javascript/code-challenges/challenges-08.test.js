...
const sortByChildren = (charArray) => {
  // Solution code here...
  charArray.sort((a,b) => b.children.length===a.children.length?a.house.localeCompare(b.house):a.children.length - b.children.length);
  return charArray;
};
...
const isCapitalized = (str) => {
  // Solution code here...
  const cap = /\b[A-Z][a-z]*\b/gm;
  const matches = str.match(cap);
  return matches || [];
};
...
const citiesAtoJ = (arr) => {
  // Solution code here...
  let citySorter = /^[A-J]/gmi;
  return arr.filter(city => city.match(citySorter));
};
